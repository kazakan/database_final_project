...
def parseMyInfo(pageSoup: BeautifulSoup):
    ...
    parse information in .mv_info

    My is typo of Mv

    """
    soup_my_info: BeautifulSoup = pageSoup.select_one(
        '.mv_info_area > .mv_info')

    ret = {}

    if(soup_my_info is None):  # this need login, discard this movie
        return None
    ...
    def parseSpan(span: BeautifulSoup):
        ...
        genre, nation, minute, release date
        """
...
def getDirectors(pageSoup: BeautifulSoup):
    ...
    for dir_obj in divs_dir_obj:
        ...
        img_director = dir_obj.select_one(
            ".thumb_dir > a:nth-child(1) > img:nth-child(1)")
        if img_director is not None:
            data['img'] = img_director['src']

        dir_obj_a = dir_obj.select_one(
            ".dir_product > a:nth-child(1)", href=True)
...
def getActors(pageSoup: BeautifulSoup):
    ...
    for li_obj in li_lst_people:
        ...
        data['code'] = int(href.split("=")[-1].strip())
        data['ismain'] = p_obj.select_one(
            "div:nth-child(3) > p:nth-child(1) > em:nth-child(1)").text
        span_actor_role = p_obj.select_one(
            "div:nth-child(3) > p:nth-child(2) > span:nth-child(1)")
        data['role'] = span_actor_role.text if span_actor_role else None

        # image
        img = li_obj.select_one(
...
def getPosterSrc(pageSoup: BeautifulSoup):
    ...
    """
    img_poster = pageSoup.select_one(
        "div.mv_info_area > div.poster > a:nth-child(1) > img:nth-child(1)")
    if(img_poster):
...
def parseOne(pageText: str):
    """
    driver code for multiprocessing.

    if page invalid return None else return crawl result.
    """

    if(pageText is None):
        return None
    if("영화 코드값 오류입니다" in pageText):
        return None
...
async def processParseBetweenCode(begin, end):
    """
    Process crawling with code between [begin,end) with asyncio & multiprocessing
    """

    codes = range(begin, end)
    begin = time()
    responses_future = [asyncio.ensure_future(
        getMovieDirectorActorPage(code)) for code in codes]
...
if __name__ == "__main__":
    ...
    for idx, chunk_start in enumerate(range(start, end, step)):
        ...
        ret = loop.run_until_complete(
            processParseBetweenCode(chunk_start, chunk_end))
